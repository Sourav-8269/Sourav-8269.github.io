import React from 'react'
import AOS from "aos";
import "aos/dist/aos.css"
import { useEffect } from "react";
import { Heading } from '@chakra-ui/react';

const Skills = () => {
  useEffect(()=>{
    AOS.init({delay:400});
},[])
  return (
    <div name="Skills"  padding="5%" >
      {/* <h1>Projects</h1> */}
      <Heading   padding="5%" fontFamily="sans-serif" data-aos="zoom-in-up" fontWeight="500">ABOUT ME</Heading>
      <div data-aos="zoom-in">
      <div  style={{display:"grid",marginTop:"0%",gridTemplateColumns:"repeat(auto-fit,minmax(150px,min-content))",justifyContent:"center",gap:"2%",width:"80%",border:"1px solid red",margin:"auto"}}>
        <div data-aos="fade-right" style={{border:"1px solid red",width:"120px",backgroundColor:"#7c7f8a",borderRadius:"15px",height:"140px",marginLeft:"5%",padding:"10%" }} >
          <img src="https://debobrota-haldar.netlify.app/static/media/html.8664168d5bc90b1a24396f85aadbf623.svg" alt="" />
          <p>HTML</p>
        </div>
        <div data-aos="fade-right" style={{width:"120px",backgroundColor:"#9b9ea8",borderRadius:"15px",height:"140px",marginLeft:"5%",padding:"10%" }}>
          <img src="https://debobrota-haldar.netlify.app/static/media/css.39769a8bd378bf410d6b381a8d88fa35.svg" alt="" />
          <p>CSS</p>
        </div>
        <div data-aos="fade-right" style={{border:"1px solid red",width:"120px",backgroundColor:"#aaafbe",borderRadius:"15px",height:"140px",marginLeft:"5%",padding:"10%" }}>
          <img src="https://debobrota-haldar.netlify.app/static/media/javascript.4c1b5332c1b1057928f6f06cf972c91c.svg" alt="" />
          <p>JavaScript</p>
        </div>
        <div data-aos="fade-right" style={{width:"120px",backgroundColor:"#9b9ea8",borderRadius:"15px",height:"140px",marginLeft:"5%",padding:"10%" }}>
          <img src="https://debobrota-haldar.netlify.app/static/media/react.4e0c97009b885bfa75c5412262a46f5f.svg" alt="" />
          <p>React</p>
        </div>
        <div data-aos="fade-right" style={{border:"1px solid red",width:"120px",backgroundColor:"#aaafbe",borderRadius:"15px",height:"140px",marginLeft:"5%",padding:"10%" }}>
          <img src="https://debobrota-haldar.netlify.app/static/media/redux.7fe607aacc31f6e6199e70f16aae407e.svg" alt="" />
          <p>Redux</p>
        </div>
        <div data-aos="fade-right" style={{border:"1px solid red",width:"120px",backgroundColor:"#aaafbe",borderRadius:"15px",height:"140px",marginLeft:"5%",padding:"10%" }}>
          <img src="https://debobrota-haldar.netlify.app/static/media/typescript.17936c69824e11a996c4.png" alt="" />
          <p>TypeScript</p>
        </div>
        <div data-aos="fade-left" style={{border:"1px solid red",width:"120px",backgroundColor:"#7c7f8a",borderRadius:"15px",height:"140px",marginLeft:"5%",padding:"10%" }} >
          <img src="https://debobrota-haldar.netlify.app/static/media/chakra.15320604467d004df871.png" alt="" />
          <p>Chakra UI</p>
        </div>
        <div data-aos="fade-left" style={{width:"120px",backgroundColor:"#9b9ea8",borderRadius:"15px",height:"140px",marginLeft:"5%",padding:"10%" }}>
          <img src="https://debobrota-haldar.netlify.app/static/media/nodejs.10c9e2aa0ca7edf36fceae25fec038a1.svg" alt="" />
          <p>Node JS</p>
        </div>
        <div data-aos="fade-left" style={{border:"1px solid red",width:"120px",backgroundColor:"#aaafbe",borderRadius:"15px",height:"140px",marginLeft:"5%",padding:"10%" }}>
          <img src="https://debobrota-haldar.netlify.app/static/media/express.4925c0127df8857c18b38329795ce88f.svg" alt="" />
          <p>Express</p>
        </div>
        <div data-aos="fade-left" style={{width:"120px",backgroundColor:"#9b9ea8",borderRadius:"15px",height:"140px",marginLeft:"5%",padding:"10%" }}>
          <img src="https://debobrota-haldar.netlify.app/static/media/mongodb.6835c61a518a732eb8318d43dfe8f700.svg" alt="" />
          <p>MongoDB</p>
        </div>
        <div data-aos="fade-left" style={{border:"1px solid red",width:"120px",backgroundColor:"#aaafbe",borderRadius:"15px",height:"140px",marginLeft:"5%",padding:"10%" }}>
          <img src="https://debobrota-haldar.netlify.app/static/media/git.c08353563539c6459b353f29d8dd7bf2.svg" alt="" />
          <p>Git</p>
        </div>
        <div data-aos="fade-left" style={{border:"1px solid red",width:"120px",backgroundColor:"#aaafbe",borderRadius:"15px",height:"140px",marginLeft:"5%",padding:"10%" }}>
          <img src="https://debobrota-haldar.netlify.app/static/media/npm.87689eab730642c86d2b01f3ef4a19e3.svg" alt="" />
          <p>npm</p>
      </div>
      <div data-aos="fade-right" style={{width:"120px",backgroundColor:"#9b9ea8",borderRadius:"15px",height:"140px",marginLeft:"5%",padding:"10%" }}>
          <img src="https://debobrota-haldar.netlify.app/static/media/database.24991eee32623f453b1c.png" alt="" />
          <p>DS & Algo</p>
        </div>
        <div data-aos="fade-right" style={{border:"1px solid red",width:"120px",backgroundColor:"#aaafbe",borderRadius:"15px",height:"140px",marginLeft:"5%",padding:"10%" }}>
          <img style={{height:"80px"}} src="https://debobrota-haldar.netlify.app/static/media/cypress.bb8c9887bc48838c8d07.webp" alt="" />
        
          <p>Cypress</p>
        </div>
        <div data-aos="fade-right" style={{marginBottom:"40%",width:"120px",backgroundColor:"#5b6378",borderRadius:"15px",height:"140px",marginLeft:"5%",padding:"10%" }}>
          <img src="data:image/png;base64," alt="" />
          <p>Postman</p>
      </div>
      </div>
      </div>
    </div>
  )
}

export default Skills